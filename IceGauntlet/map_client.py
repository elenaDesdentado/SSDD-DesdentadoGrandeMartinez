...
class MapClient(Ice.Application):
    ...
    def run(self, argv):
        ...
        if options.operation.lower() == OPTION_PUBLISH_MAP:
            ...
            with open(map_file, 'r', encoding='UTF-8') as map_hile_handler:
                map_dictionary = json.load(map_hile_handler)
                room_name = map_dictionary['room']
                try:
                    map_server.remove(token, room_name)
                except IceGauntlet.Unauthorized:
                    print(
